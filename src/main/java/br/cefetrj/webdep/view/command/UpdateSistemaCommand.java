...
public class UpdateSistemaCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			int novaDias = Integer.parseInt(request.getParameter("novaLeituraDia")); // MEXI
																						// AQUI
			String novaHora = request.getParameter("novaLeituraHora"); // MEXI
																		// AQUI
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm"); // MEXI
																				// AQUI
			LocalTime tempoNovaLeitura = LocalTime.parse(novaHora, formatter); // MEXI
																				// AQUI
			LocalDate ld = LocalDate.parse(request.getParameter("dataPrimeiraLeitura")); // MEXI
																							// AQUI
			LocalTime lt = LocalTime.parse(request.getParameter("horaPrimeiraLeitura")); // MEXI
																							// AQUI
			LocalDateTime l = LocalDateTime.of(ld, lt);

			GregorianCalendar c = new GregorianCalendar();
			c.set(Calendar.DAY_OF_YEAR, novaDias); // MEXI AQUI
			c.set(Calendar.HOUR_OF_DAY, tempoNovaLeitura.getHour()); // MEXI
																		// AQUI
			...
			switch (lang) {
			case "en_US":
				mensagem = "Error! Please check the dates and the hours on the form!";
				break;

			case "pt_BR":
				mensagem = "Erro! Por favor checar se as datas e horários selecionados são válidos!";
				break;
